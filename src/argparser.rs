...
impl ArgParser {
    ...
    /// }
    /// ```
    pub fn parse<'a, I: Iterator<Item = &'a String>>(&self, args: I, verbose: bool) -> ParseResult {
        ...
        for (argname, my_arg) in self.arguments.iter() {
            for (flag, rest) in argvec.slide().filter(|&(f, _)| {
                f == &format!("-{}", my_arg.flag) || f == &format!("--{}", argname)
            }) {
                if let Entry::Occupied(mut e) = new_args.entry(argname.clone()) {
                    ...
                    match arg.type_ {
                        ArgType::Flag => {
                            arg.val = Some("true".into());
                        }
                        ArgType::Option => {
                            let err = format!(
                                "This option `{}` requires a value you have not provided",
                                argname
                            );
                        ...
                        ArgType::List | ArgType::Dict => {
                            if let Some(rest) = rest {
                                ...
                                taken_up.extend(
                                    rest.iter().take_while(|x| !(is_flag(x) || is_long_flag(x))),
                                );
                            } else {
                                let err = format!(
                                    "This option `{}` requires a value you have not provided",
        ...
        for (_, ref mut v) in new_args
            .iter_mut()
            .filter(|&(_, ref vv)| vv.val.is_none() && vv.type_.is_positional())
        {
            if let Some((_, x)) = argvec
                .iter()
                .skip(1)
    ...
    V: FromStr,
{
    s.split_whitespace()
        ...
        .map(|(k, v)| {
            k.parse()
                .ok()
                .and_then(|k2| v.parse().ok().map(|v2| (k2, v2)))
        })
        .enumerate()
        .fold(None, |acc, (idx, elem)| {
...
#[cfg(test)]
mod test {
    use super::{hashmap_parser, vec_parser, ArgParser, ArgType};
    use std::collections::HashMap;
    ...
    fn test_parser() {
        ...
        let test_1 = "./go -l -60 -h -6001.45e-2 -n Johnny --mao -f 1 2 3 4 5"
            .split_whitespace()
            .map(|s| s.into())
            .collect::<Vec<String>>();

        let p_res = parser.parse(test_1.iter(), true).unwrap();
        ...
        assert_eq!(
            p_res.get_with("frequencies", vec_parser),
            Some(vec![1, 2, 3, 4, 5])
        );
    ...
    fn test_parser_unrequired() {
        ...
        let test_1 = "./go -l -60 -h -6001.45e-2 -n Johnny -f 1 2 3 4 5"
            .split_whitespace()
            .map(|s| s.into())
            .collect::<Vec<String>>();
    ...
    fn test_parser_unrequired_nodefault() {
        ...
        let test_1 = "./go -l -60 -h -6001.45e-2 -n Johnny"
            .split_whitespace()
            .map(|s| s.into())
            .collect::<Vec<String>>();
        ...
        assert_eq!(p_res.get::<String>("name"), Some("Johnny".into()));
        assert_eq!(
            p_res.get_with::<Vec<u8>, _>("frequencies", vec_parser),
            None
        );
        assert_eq!(p_res.get("mao"), Some(false));
    ...
    fn test_parser_dict() {
        ...
        let test_1 = "./go -l -60 -h -6001.45e-2 -n Johnny -s Monday:true Friday:false"
            .split_whitespace()
            .map(|s| s.into())
            .collect::<Vec<String>>();
        ...
        assert_eq!(
            p_res.get_with::<HashMap<String, bool>, _>("socks", hashmap_parser),
            Some(h)
        );
    ...
    fn test_parser_positional() {
        ...
        let test_1 = "./go -l -60 -h -6001.45e-2 -n Johnny crap.csv crap.json"
            .split_whitespace()
            .map(|s| s.into())
            .collect::<Vec<String>>();
